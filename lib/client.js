...
body.dsh-skin-galactic-opera .dsh-skin-telemetry-key {
  ...
  color: rgba(125,227,255,0.82);
  font-size: 9px;
  line-height: 1.25;
  letter-spacing: 0.08em;
}

body.dsh-skin-galactic-opera .dsh-skin-telemetry-key::after {
  content: ":";
  color: rgba(255,207,92,0.70);
  margin-left: 2px;
}
...
`;
		...
		function installTelemetryPanel() {
			...
			const scoreCandidate = (element) => {
				...
				if (text.length < 2 || text.length > 360 || !telemetryPattern.test(text)) return 0;
			...
			const splitTelemetryRows = (text) => {
				const normalized = text.replace(/\s*([·•|；;])\s*/g, " | ").replace(/\s{2,}/g, " ").trim();
				let parts = normalized.split(/\s+\|\s+|\s+·\s+|\s+•\s+|\s*；\s*|\s*;\s*/).map((part) => part.trim()).filter(Boolean);
				if (parts.length <= 1) {
					const metricPattern = /(?:^|\s)((?:(?:prompt|completion|cached?|total|context|usage|model|cost|rounds?|turns?|steps?|tokens?|toks?|调用|消耗|上下文|模型|费用|缓存|命中|输入|输出|轮次|剩余轮次|剩余步骤|步骤|步数|轮|步)[\w\s\u4e00-\u9fa5]*?[:：]?\s*)?[\d.,KkMm%/]+\s*(?:tokens?|toks?|rounds?|turns?|steps?|轮|步)?)/gi;
					parts = [...normalized.matchAll(metricPattern)].map((match) => match[1].trim()).filter((part) => telemetryPattern.test(part));
				}
				const seen = new Set();
				return parts.map((part) => {
					let key = "";
					let value = part;
					let match = /^(.*?)[：:]\s*(.+)$/.exec(part);
					if (match !== null) {
						key = match[1].trim();
						value = match[2].trim();
					} else if ((match = /^([^\d]+?)\s+([\d.,KkMm%/]+(?:\s*(?:tokens?|toks?|rounds?|turns?|steps?|轮|步))?.*)$/i.exec(part)) !== null) {
						key = match[1].trim();
						value = match[2].trim();
					} else if ((match = /^([\d.,KkMm%/]+)\s*(tokens?|toks?|rounds?|turns?|steps?|轮|步)$/i.exec(part)) !== null) {
						key = match[2].trim();
						value = match[1].trim();
					} else if ((match = /^([\d.,KkMm%/]+(?:\s*(?:tokens?|toks?|rounds?|turns?|steps?|轮|步))?)\s+(.+)$/i.exec(part)) !== null) {
						key = match[2].trim();
						value = match[1].trim();
					}
					if (key === "") {
						const unitMatch = /^([\d.,KkMm%/]+)\s*(tokens?|toks?|rounds?|turns?|steps?|轮|步)$/i.exec(value);
						if (unitMatch !== null) {
							key = unitMatch[2].trim();
							value = unitMatch[1].trim();
						}
					}
					const signature = `${key}\u0000${value}`;
					if (seen.has(signature)) return undefined;
					seen.add(signature);
					return { key, value };
