...
`;
		...
		function installTelemetryPanel() {
			...
			const splitTelemetryRows = (text) => {
				...
				const labelBoundary = /\s+(?=(?:LLM\b|工具调用|TOKEN\s*平均|TOK\/S|缓存命中|缓存|命中|模型|费用|上下文|输入|context\b|usage\b|model\b|cost\b|cache\b|cached\b|prompt\b|tokens?\b|toks?\b))/gi;
				const parts = strongParts.flatMap((part) => part
