...
`;
		...
		function installInteractions(decor) {
			...
			const updatePointer = () => {
				frame = 0;
				lastPointerPaintAt = typeof performance !== "undefined" ? performance.now() : Date.now();
			...
			const onPointerMove = (event) => {
				...
				if (frame !== 0 || now - lastPointerPaintAt < 32) return;
		...
		function installTelemetryPanel() {
			...
			const getTelemetryText = (element) => (element.textContent || "").replace(/\s+/g, " ").trim();
