...
body.dsh-skin-galactic-opera .dsh-skin-telemetry-key {
  color: rgba(125,227,255,0.78);
  font-size: 8px;
  line-height: 1.1;
...
`;
		...
		function installTelemetryPanel() {
			...
			const telemetryPattern = /token|tokens|调用|消耗|上下文|context|usage|prompt|completion|cache|cached|模型|model|费用|cost|轮次|round/i;
			const skipSelector = `#${DECOR_ID}, style, script, textarea, input, select, [contenteditable=true], .dsh-skin-pulse, .dsh-skin-telemetry-panel`;
			...
			const scoreCandidate = (element) => {
				...
				if (rect.width < 80 || rect.height < 10 || rect.width > Math.min(620, window.innerWidth * 0.72) || rect.height > 72) return 0;
				if (rect.bottom < window.innerHeight - 150) return 0;
				const text = getTelemetryText(element);
				if (text.length < 8 || text.length > 180 || !telemetryPattern.test(text)) return 0;
				let score = 1;
				if (rect.bottom > window.innerHeight - 72) score += 3;
				if (rect.right > window.innerWidth * 0.45) score += 1;
				if (/token|tokens/i.test(text)) score += 3;
				if (/调用|消耗|上下文|usage|context/i.test(text)) score += 2;
				if (/button|input|textarea|select/i.test(element.tagName)) score -= 4;
				return score;
			};
			const chooseTelemetryElement = () => {
				const points = [
					[window.innerWidth * 0.25, window.innerHeight - 18],
					[window.innerWidth * 0.50, window.innerHeight - 18],
					[window.innerWidth * 0.75, window.innerHeight - 18],
					[window.innerWidth * 0.50, window.innerHeight - 44],
					[window.innerWidth * 0.75, window.innerHeight - 44]
				];
				const candidates = new Map();
				for (const [x, y] of points) {
					for (const base of document.elementsFromPoint(x, y)) {
						let element = base;
						for (let depth = 0; element instanceof HTMLElement && depth < 5; depth += 1, element = element.parentElement) {
			...
			const splitTelemetryRows = (text) => {
				const normalized = text.replace(/\s*([·•|；;])\s*/g, "|$1|").replace(/\s{2,}/g, " ").trim();
				let parts = normalized.split(/\|[·•|；;]\|/).map((part) => part.trim()).filter(Boolean);
				if (parts.length <= 1) parts = normalized.split(/\s+\/\s+|\s+·\s+|\s+•\s+/).map((part) => part.trim()).filter(Boolean);
				if (parts.length <= 1) {
					const matches = [...normalized.matchAll(/(?:^|\s)((?:prompt|completion|cached?|total|context|usage|model|cost|round|token|tokens|调用|消耗|上下文|模型|费用|轮次)[\w\s\u4e00-\u9fa5]*?)([:：]?\s*[\d.,KkMm%/]+(?:\s*tokens?)?)/g)];
					parts = matches.map((match) => `${match[1].trim()} ${match[2].trim()}`);
				}
				return parts.slice(0, 8).map((part) => {
					const match = /^(.*?)(?:[:：]\s*|\s{1,})([\d.,KkMm%/]+(?:\s*tokens?)?)$/.exec(part);
					return match === null ? { key: "DATA", value: part } : { key: match[1].trim() || "DATA", value: match[2].trim() };
				});
			};
			const renderPanel = (text) => {
				const rows = splitTelemetryRows(text);
				panel.hidden = rows.length === 0;
				panel.replaceChildren(...rows.map(({ key, value }) => {
					const row = document.createElement("div");
					row.className = "dsh-skin-telemetry-row";
					const label = document.createElement("span");
					label.className = "dsh-skin-telemetry-key";
					label.textContent = key;
					const metric = document.createElement("span");
					metric.className = "dsh-skin-telemetry-value";
			...
			const update = () => {
				frame = 0;
				const next = chooseTelemetryElement();
				const text = next === undefined ? "" : getTelemetryText(next);
				if (next !== currentSource) {
					currentSource?.classList.remove("dsh-skin-telemetry-source-hidden");
					currentSource = next;
					currentSource?.classList.add("dsh-skin-telemetry-source-hidden");
				}
