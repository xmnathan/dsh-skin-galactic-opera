		...
		function installTelemetryPanel() {
			...
			const splitTelemetryRows = (text) => {
				...
				const isTelemetryRow = (value) => {
					...
					return /(?:\d[\d.,KkMm%/]*\s*轮\s*[·•]?\s*\d[\d.,KkMm%/]*\s*步)|(?:\d[\d.,KkMm%/]*\s*(?:rounds?|turns?)\s*[·•]?\s*\d[\d.,KkMm%/]*\s*steps?)|^LLM\b|^工具调用|^(?:首\s*)?(?:token|tokens|tok|toks)\s*平均\b|^(?:缓存命中|缓存|cache\b|cached\b)|^(?:输入|输出|prompt\b|completion\b)|^(?:上下文|context\b|usage\b|模型|model\b|费用|cost\b)/i.test(value);
				};
				...
				const firstTokenRow = firstMatch((row) => /^(?:首\s*)?(?:token|tokens|tok|toks)\s*平均\b/i.test(row));
