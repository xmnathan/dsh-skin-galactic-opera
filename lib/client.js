...
`;
		...
		function installTelemetryPanel() {
			...
			const scoreCandidate = (element) => {
				if (!(element instanceof HTMLElement) || isSkippable(element)) return 0;
				const rect = element.getBoundingClientRect();
				if (rect.width < 80 || rect.height < 12 || rect.width > window.innerWidth * 0.9 || rect.height > 160) return 0;
				if (rect.bottom < window.innerHeight - 140) return 0;
				const text = (element.innerText || element.textContent || "").replace(/\s+/g, " ").trim();
				if (text.length < 8 || text.length > 260 || !telemetryPattern.test(text)) return 0;
				let score = 1;
